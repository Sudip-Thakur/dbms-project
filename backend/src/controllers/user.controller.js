...
const registerUser = asyncHandler(async (req, res)=>{
  ...
  const existedUser = await sql `select * from users where users.username = ${username} or users.email = ${email}`
  ...
  const createdUser = await sql`insert into users(username, email, fullname, password, bio) 
...
const loginUser = asyncHandler(async(req, res)=>{
  ...
  const { accessToken , refreshToken} = await generateAccessAndRefereshTokens(user[0].id)

  const loggedInUser = await sql`select username, email, fullName from users where id = ${user[0].id}` 
...
const changePassword = asyncHandler(async (req, res)=>{
  ...
  const user = await sql `select password from users where id=${req.user[0]?.id}`
  console.log(user);
...
const updateFullName = asyncHandler(async (req, res)=>{
  ...
  const user = await sql `update users set fullname=${fullname} where id=${req.user[0]?.id}
...
const updateAvatar = asyncHandler(async (req, res)=>{
  ...
  const currentAvatarUrl = await sql `select avatar from users where id=${req.user[0]?.id}`
  const result = await sql `update users set avatar=${avatar.url} where id=${req.user[0]?.id} 
